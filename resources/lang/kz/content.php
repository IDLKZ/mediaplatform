<?php

return [
    //subscribers
    'request' => 'Өтініштер',
    'listeners' => 'Тыңдаушылар',
    'course_title' => 'Курстың аты',
    'sendTime' => 'Сұрау жіберу уақыты',
    'approved' => 'Мақұлданды',
    'requested' => 'Мақұлданбады',
    'accept'=>"Қабылдау",
    'deny'=>'Бас тарту',
    'detail'=>"Толығырақ",
    //Left SideBar (Левое меню)
    "sidebar.index"=>"Басты бет",
    "sidebar.users"=>"Тындаушылар",
    "sidebar.tasks"=>"Тапсырмалар",
    "sidebar.media"=>"Медиа",
    "sidebar.news"=>"Жаңалықтар",
    "sidebar.examination"=>"Емтихандар",
    "sidebar.request"=>"Сұраулар",
    "sidebar.search"=>"Іздеу",
    "sidebar.forum"=>"Форум",
    "sidebar.logout"=>"Шығу",

    //Header`s Add Button - Добавить новые материалы
    "header.new_course"=>"Жаңа Курс",
    "header.new_video"=>"Жаңа Бейнесабақ",
    "header.new_user"=>"Жаңа Пайдаланушы",
    "header.new_material"=>"Жаңа Материал",
    "header.new_examination"=>"Жаңа Емтихан",
    "header.new_news"=>"Жаңа Жаңалық",
    "header.new_question"=>"Тестке Жаңа сұрақ",
    "header.new_reviewquestion"=>"Сауалнамаға жаңа сұрақ",

    //Header Profile - Профиль админа
    "profile.my_profile"=>"Менің профилім",
    "profile.settings"=>"Баптаулар",
    "profile.request"=>"Өтініштерім",
    "profile.logout"=>"Шығу",
    //User
    "user.teachers"=>"Тьюторлар",
    "user.teacher"=>"Тьютор",
    "user.about_teacher"=>"тьютор Туралы",
    "user.teacher_description"=>"Мұнда сіз бейне сабақтарын жүргізетін тьюторлардың тізімін көре аласыз",
    "user.all_teachers"=>"Барлық тьюторлар",

    //course
    "course"=>"Курс",
    'new_course'=>"Жаңа видеокурс",
    'course_subtitle'=>"Курс тақырыпшасы",
    "course_description"=>"Курстың сипаттамасы",
    "course_advantages"=>"Курстың артықшылықтары",
    "course_requirements"=>"Курстың талаптары",
    "course_language"=>"Курс тілі",
    "course_image"=>"Курстың суреті",
    "course_status"=>"Курс шығаруға дайын",
    "img"=>"Сурет",
    "list_courses"=>"Менің видеокурстарым",
    "small_subtitle_courses"=>"Мұнда сіз бейне курстарыңызды көре аласыз",
    "course_last_update"=>"Соңғы жаңарған уақыты",
    "not_course"=>"Әзірге курс жоқ",
    "course_info"=>"Информация о Курсе",
    "course_author"=>"Курс авторы",
    "course_lang"=>"Негізгі тіл",
    "course_about"=>"Курс туралы",
    "course_skills"=>"Дағдылар",
    "course_requirement"=>"Талаптар",
    "course_what_will_you_learn"=>"Бұл курстан не үйренесің",
    "course_change"=>"Видеокурсты өзгерту",

    //video
    "video"=>"Видео",
    "video_title"=>"Видео аты",
    'new_video'=>"Жаңа видео",
    "video_count"=>"Видео номері",
    "video_video"=>"Видео",
    "video_course"=>"Курстың видеосы",
    "video_preview"=>"Алдын ала қарауға рұқсат етілген",
    "my_video"=>"Менің видеосабақтарым",
    "video_allowed"=>"Превью",
    "video_notallowed"=>"Не превью",
    "selected" => "Таңдалмады",
    "video_materials" => "Материалдар мен анықтамалар",
    "video_explanation" => "Видеоның түсініктемелері мен сипаттамалары",
    'videoCourse' => 'Курстың видеосабақтары',
    'notVideoCourse' => 'Курстың видеосабақтары жоқ',

    //material
    "material"=>"Материал",
    "material_my"=>"Менің материалдарым",
    "filename"=>"Файл атауы",
    "links"=>"Сілтемелер",
    "not_links"=>"Сілтеме жоқ",
    "material_type"=>"Материал түрі",
    "material_video"=>"Материалға файлдар",
    "material_add"=>"Видеоға жаңа материал қосу",
    "additional_links"=>"Қосымша сілтемелер",
    'material_audio' => 'Аудио жазба',
    'material_other' => 'Басқа',

    //results
    "result_list" => 'Тыңдаушылардың нәтижелері',
    "result_time_pass" => 'Емтихан тапсырған уақыты',

    //examination
    "examination"=>"Емтихандар",
    "new_examination"=>"Жаңа емтихандар",
    "examination_type"=>"Емтихан түрі",
    "examination_base"=>"Сұрақтар базасы",
    "examination_description"=>"Емтиханға сипаттама",
    "examination_material"=>"Емтиханға арналған материалдар",



    //buttons
    'save' => 'Сақтау',
    'change'=>"Өзгерту",
    'back' => 'Артқа',
    'clear' => 'Тазалау',
    'delete' => 'Жою',
    'cancel' => 'Болдырмау',
    "edit"=>"Редакциялау",
    "show"=>"Толығырақ",
    "watch"=>"Көру",
    "action"=>"Әрекеттер",
    "download"=>"Жүктеу"



];

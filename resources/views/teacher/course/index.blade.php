                ...
                <div class="col-md-6 col-sm-12 col-xs-12">
                    <div class="boxs project_widget">
                        ...
                        <div class="pw_content">
                            <div class="pw_header">
                                <h6>{{$course->title}}</h6>
                                <small class="text-muted">{{\Illuminate\Support\Facades\Auth::user()->name}}  |
                                    {{__('content.course_last_update')}}: {{\Illuminate\Support\Carbon::parse($course->updated_at)->diffForHumans()}}</small>
                            </div>
                            <div class="pw_meta">
                                <a href="{{route("course.show",$course->alias)}}"><span>{{__('content.watch')}}</span></a>
                                <a href="{{route("course.edit",$course->alias)}}"><small class="text-muted">{{__('content.edit')}}</small></a>
